/**
 * Generate the blog-post SVG charts from data/sweep-results.json.
 * Light-surface palette (validated): baseline/today #4A72C4, upgrade #C98500;
 * stacked components use same-hue blues. Icelandic labels, comma decimals.
 * Output: ../3b/public/img/umferd/*.svg
 */
...
<text x="16" y="41" fill="${INK2}" font-size="12">ma.kr. á ári (fyrsta ár) — tímasparnaður + slysaábati, líkanið umferð.</text>`;
...
<rect x="${left}" y="${y}" width="${Math.max(t, 1.5)}" height="${barH}" rx="3" fill="${BLUE}"/>`;
      if (s > 1) body += `<rect x="${left + t + 2}" y="${y}" width="${s}" height="${barH}" rx="3" fill="${BLUE_DARK}"/>`;
  ...
  y += 14;
  body += `<rect x="${left}" y="${y}" width="12" height="12" rx="2" fill="${BLUE}"/><text x="${left + 18}" y="${y + 10}" fill="${INK2}" font-size="12">Tímasparnaður</text>
<rect x="${left + 130}" y="${y}" width="12" height="12" rx="2" fill="${BLUE_DARK}"/><text x="${left + 148}" y="${y + 10}" fill="${INK2}" font-size="12">Slysaábati</text>`;
...
<text x="16" y="41" fill="${INK2}" font-size="12">Fjöldi ferða eftir ferðatíma — í dag (blátt) og eftir 2+2 + 110 (gult)</text>
...
<rect x="${left}" y="${y + barH + 6}" width="${sX(v2.benefits.totalISKPerYear)}" height="${barH}" rx="3" fill="${BLUE}"/>
...
<rect x="${left}" y="${y}" width="${Math.max(sX(r.perKm), 1.5)}" height="${barH}" rx="3" fill="${BLUE}"/>
...
<rect x="40" y="${roadY - 34}" width="680" height="68" rx="6" fill="#F3F4F6"/>
<line x1="40" y1="${roadY}" x2="720" y2="${roadY}" stroke="#D1D5DB" stroke-width="2" stroke-dasharray="14 10"/>`;
...
<text x="${truckX + 22}" y="${roadY + 48}" text-anchor="middle" fill="${BLUE_DARK}" font-size="11" font-weight="700">Flutningabíll á 78 km/klst</text>`;
  for (let i = 0; i < 5; i++) {
    body += `<rect x="${truckX - 34 - i * 36}" y="${roadY + 10}" width="${carW}" height="${carH}" rx="3" fill="${BLUE}"/>`;
  }
...
<text x="${truckX - 116}" y="${roadY + 58}" text-anchor="middle" fill="${INK2}" font-size="11">bílalest — allir á hraða flutningabílsins</text>`;
  ...
  for (const x of [120, 250, 560]) {
    body += `<rect x="${x}" y="${roadY - 24}" width="${carW}" height="${carH}" rx="3" fill="#9CA3AF"/>`;
  }
